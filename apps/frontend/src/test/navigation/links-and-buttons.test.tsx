...
describe('Navigation — liens du layout', () => {
  ...
  it('liens directs : tableau de bord, flux, connecteurs, mappings, marketplace, admin ANEXYS', () => {
    ...
    expect(screen.getByRole('link', { name: /admin anexys/i })).toHaveAttribute('href', '/backoffice');
  });

  it('menu Monitoring : alertes vers /monitoring, hub pipeline vers /hub/pipeline', async () => {
  ...
  it('menu Planifier : lien vers /planifier', async () => {
    const user = userEvent.setup();
    renderPrivateLayout('/dashboard');
    await user.click(screen.getByRole('button', { name: /^planifier$/i }));
    await waitFor(() => {
      expect(screen.getByRole('menuitem', { name: /liste des planifications/i })).toBeInTheDocument();
    });
    expect(screen.getByRole('menuitem', { name: /liste des planifications/i })).toHaveAttribute('href', '/planifier');
  });
